...
def binary_to_image(bytes_msg, offset=0):
    ...
    img_data = np.frombuffer(bytearray(bytes_msg), dtype=np.uint8, offset=offset + metadata_length * 4)
...
def encode_inference_settings(steps=20, width=256, context_pad=150, cfg_weight=4.0, tg_weight=0.0, tg_steps=0):
