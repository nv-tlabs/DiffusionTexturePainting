...
from utilities import Engine, device_view, save_image, GpuAllocator
...
class StableDiffusionPipeline:
    ...
    def __init__(
        ...
        Args:
            ...
            texture_guidance_steps (int):
                The number of texture guidance steps.
                More texture guidance steps lead to more repetitive texture pattern
            scheduler (str):
    ...
    def loadResources(self, image_height, image_width, batch_size, seed):
        ...
        self.set_sizes(image_height=image_height, image_width=image_width, batch_size=batch_size)

    def set_seed(self, seed):
        # Initialize noise generator
        self.generator = torch.Generator(device="cuda").manual_seed(seed) if seed else None

    def set_sizes(self, image_height, image_width, batch_size):
    ...
    def cachedModelName(self, model_name):
        # HACK: no special inpaint version of material decoder
        if self.inpaint and model_name != "matvae":
            model_name += '_inpaint'
        return model_name
    ...
    def loadEngines(
        ...
        if 'unet' in self.stages:
            self.models['unet'] = make_UNet(inpaint=self.inpaint, text_maxlen=text_maxlen, lora_path=lora_path,
                                            **models_args)
        ...
        self.shared_device_memory = allocator.allocate(max_device_memory)
    ...
    def denoise_latent(self, latents, text_embeddings, timesteps=None, step_offset=0, mask=None,
                       masked_image_latents=None):
        cudart.cudaEventRecord(self.events['denoise-start'], 0)
        if not isinstance(timesteps, torch.Tensor):
            timesteps = self.scheduler.timesteps

        texture_guidance_scale = self.texture_guidance_scale
        for step_index, timestep in enumerate(timesteps):
            assert not torch.isnan(latents).any()
            if self.nvtx_profile:
                nvtx_latent_scale = nvtx.start_range(message='latent_scale', color='pink')

            if step_index > self.texture_guidance_steps - 1:
                texture_guidance_scale = 0.0

            # Expand the latents if we are doing classifier free guidance
            latent_model_input = torch.cat([latents] * 3)
            latent_model_input = self.scheduler.scale_model_input(latent_model_input, step_offset + step_index,
                                                                  timestep)
            ...
            # Perform guidance
            noise_pred_uncond, noise_pred_text, noise_pred_tg = noise_pred.chunk(3)
            noise_pred = noise_pred_uncond + self.guidance_scale * (
                        noise_pred_text - noise_pred_uncond) + texture_guidance_scale * (noise_pred_tg - noise_pred_text)
    ...
    def print_summary(self, denoising_steps, tic, toc, vae_enc=False):
        ...
        print('|------------|--------------|')
        if vae_enc:
            print('| {:^10} | {:>9.2f} ms |'.format('VAE-Enc',
                                                    cudart.cudaEventElapsedTime(self.events['vae_encoder-start'],
                                                                                self.events['vae_encoder-stop'])[1]))
        print('| {:^10} | {:>9.2f} ms |'.format('CLIP', cudart.cudaEventElapsedTime(self.events['clip-start'],
                                                                                    self.events['clip-stop'])[1]))
        print('| {:^10} | {:>9.2f} ms |'.format('UNet x ' + str(denoising_steps),
                                                cudart.cudaEventElapsedTime(self.events['denoise-start'],
                                                                            self.events['denoise-stop'])[1]))
        print('| {:^10} | {:>9.2f} ms |'.format('VAE-Dec', cudart.cudaEventElapsedTime(self.events['vae-start'],
                                                                                       self.events['vae-stop'])[1]))
        print('|------------|--------------|')
        print('| {:^10} | {:>9.2f} ms |'.format('Pipeline', (toc - tic) * 1000.))
        print('|------------|--------------|')

    def save_image(self, images, pipeline, prompt, normalize_images=True):
        # Save image
        image_name_prefix = pipeline + '-fp16' + ''.join(
            set(['-' + prompt[i].replace(' ', '_')[:10] for i in range(len(prompt))])) + '-'
        image_paths, images = save_image(images, self.output_dir, image_name_prefix, normalize_images)
        print(f"[stable_diffiusion_pipeline.save_image] image_paths: {image_paths}")
