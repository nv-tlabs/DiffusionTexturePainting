...
class GpuAllocator(trt.IGpuAllocator):
    def __init__(self):
        trt.IGpuAllocator.__init__(self)

    def allocate(self, size, alignment=512, flags=0):
        shape = (size,)
        return cuda.DeviceArray.raw(shape)
...
class Engine():
    ...
    def build(self, onnx_path, fp16, input_profile=None, enable_refit=False, enable_preview=False, timing_cache=None,
              workspace_size=0):
...
class DDIMScheduler():
    ...
    def step(self, model_output, sample, idx, timestep,
        ...
        alpha_prod_t_prev = self.alphas_cumprod[
            prev_idx] if prev_idx < self.num_inference_steps else self.final_alpha_cumprod
...
class PNDMScheduler():
    ...
    def step(self, output, sample, idx, timestep):
        ...
        prev_sample = (
            self.sample_coeff[idx] * sample - (self.alphas_cumprod_prev[idx] - self.alphas_cumprod[idx]) * output /
            self.model_output_denom_coeff[idx]
        )
        self.counter += 1
...
def normalize_images(images):
    ...
    """
    return ((images + 1) * 255 / 2).clamp(0, 255).detach().permute(0, 2, 3, 1).round().type(torch.uint8).cpu().numpy()


def save_image(images, image_path_dir, image_name_prefix, normalize_images=True):
    """
    Save the generated images to png files.
    """
    image_paths = []
    # if input images are unnormalized float torch tensors
    if normalize_images:
        images = ((images + 1) * 255 / 2).clamp(0, 255).detach().permute(0, 2, 3, 1).round().type(
            torch.uint8).cpu().numpy()
    # otherwise images were numpy arrays already properly normalized
    for i in range(images.shape[0]):
        image_path = os.path.join(image_path_dir,
                                  image_name_prefix + str(i + 1) + '-' + str(random.randint(1000, 9999)) + '.png')
        print(f"Saving image {i + 1} / {images.shape[0]} to: {image_path}")
        if images.shape[-1] == 1:
            # special case for grayscale (single channel) images
            Image.fromarray(images[i].squeeze(), mode="L").save(image_path)
        else:
            Image.fromarray(images[i]).save(image_path)
        image_paths.append(image_path)
    print(f"[utilities.save_image] image_paths: {image_paths}")
...
def add_arguments(parser):
    ...
    parser.add_argument('--version', type=str, default="2.1",
                        choices=["1.4", "1.5", "2.0", "2.0-base", "2.1", "2.1-base"],
                        help="Version of Stable Diffusion")
    parser.add_argument('prompt', nargs='*', help="Text prompt(s) to guide image generation")
    parser.add_argument('--negative-prompt', nargs='*', default=[''],
                        help="The negative prompt(s) to guide the image generation.")
    parser.add_argument('--repeat-prompt', type=int, default=1, choices=[1, 2, 4, 8, 16],
    ...
    # ONNX export
    parser.add_argument('--onnx-opset', type=int, default=16, choices=range(7, 18),
    ...
    parser.add_argument('--force-onnx-export', action='store_true',
                        help="Force ONNX export of CLIP, UNET, and VAE models")
    parser.add_argument('--force-onnx-optimize', action='store_true',
                        help="Force ONNX optimizations for CLIP, UNET, and VAE models")
    parser.add_argument('--onnx-minimal-optimization', action='store_true',
    ...
    parser.add_argument('--build-dynamic-shape', action='store_true',
                        help="Build TensorRT engines with dynamic image shapes.")
    parser.add_argument('--build-enable-refit', action='store_true',
                        help="Enable Refit option in TensorRT engines during build.")
    parser.add_argument('--build-preview-features', action='store_true',
                        help="Build TensorRT engines with preview features.")
    parser.add_argument('--timing-cache', default=None, type=str,
